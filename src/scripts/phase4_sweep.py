#!/usr/bin/env python3
"""Phase 4: model × rtsp-resolution sweep (ticket 032).

Drops the imgsz axis (existing .engine files only — all at imgsz=640).
Full 3×3 cross product of:
- model: yolov11{n,s,m}-drone-640x640.engine (square imgsz=640 baseline)
- rtsp_camera WxH: 960×540, 640×360, 480×270

For each cell:
1. Launch rtsp_camera (with resolution set).
2. Launch tracker_node (with model set).
3. Wait WARMUP_SEC for TensorRT engine load + first-infer settling.
4. Probe stamp_age on /$ROBOT_NAME/yolo_result_vision for DURATION_SEC.
5. Tear both down cleanly (process-group kill) and reap.

Requires:
- Environment already sourced (ROS2 + mas workspace + robot.env).
- `rtsp_camera_node` and `tracker_node` NOT currently running — the
  script refuses to start if it detects them in `ros2 node list`,
  since overlapping nodes corrupt the per-cell measurement.
- A8 streaming at $RTSP_URL.
"""
...
MODELS = [
    "yolov11n-drone-640x640.engine",
    "yolov11s-drone-640x640.engine",
    "yolov11m-drone-640x640.engine",
]
RESOLUTIONS = [(960, 540), (640, 360), (480, 270)]


def check_conflicting_nodes():
    """Abort early if rtsp_camera or tracker are already alive."""
    try:
        out = subprocess.run(
            ["ros2", "node", "list"],
    ...
    if conflict:
        ...
        for n in conflict:
            print(f"  {n}")
        print("\nKill the rtsp_camera and tracker tmux panes (Ctrl-C) and re-run.")
        sys.exit(2)


def spawn(cmd, log_path):
    """Spawn in its own process group so we can killpg on teardown."""
...
def parse_infer(log_path):
    """Pull the final 'infer N=... p50=X p95=Y' line from tracker log."""
    try:
        last = None
...
def main():
    ...
    print(f"# Phase 4 model × rtsp sweep — {len(cells)} cells × "
          f"(warmup {WARMUP_SEC}s + probe {DURATION_SEC}s + teardown {TEARDOWN_SEC}s) "
          f"≈ {total_min:.1f} min")
    print(f"# ROBOT_NAME={ROBOT_NAME}  RTSP_URL={RTSP_URL}")
    print(f"# probe={PROBE_TOPIC}  csv_dir={CSV_DIR}")
    print()
    ...
    for model, w, h in cells:
        ...
        csv_path = os.path.join(CSV_DIR, f"phase4_{name}.csv")
        rtsp_log = os.path.join(CSV_DIR, f"phase4_{name}_rtsp.log")
        trk_log = os.path.join(CSV_DIR, f"phase4_{name}_tracker.log")
